...
type CatalogoExame = { id: number; nome: string };
type CatalogoLocal = { id: number; nome: string };
...
export default function AgendamentosPage() {
  ...
  const [, setLocais] = useState<CatalogoLocal[]>([]);
    ...
    async function run() {
      ...
      try {
        const res = await fetch(
          `/api/agendamentos?cpf=${encodeURIComponent(cpf)}&status=${encodeURIComponent(tab)}&tipo=exame`,
          { cache: "no-store" },
        );
  ...
  useEffect(() => {
    let cancelled = false;
    async function run() {
      if (step !== "novo_tipo" || exames.length) return;
      setCatalogoLoading(true);
      try {
        const res = await fetch("/api/agendamentos/catalogo", { cache: "no-store" });
        const json = (await res.json()) as { exames?: CatalogoExame[]; locais?: CatalogoLocal[]; error?: string };
        if (!res.ok) throw new Error(json.error || "Falha ao carregar catálogo");
        if (cancelled) return;
        setExames(json.exames || []);
        setLocais(json.locais || []);
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : "Erro ao carregar catálogo");
      } finally {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const res = await fetch(`/api/agendamentos/disponibilidade?exameId=${encodeURIComponent(String(selectedExameId))}`, {
          cache: "no-store",
        });
        const json = (await res.json()) as { dates?: string[]; error?: string };
        if (!res.ok) throw new Error(json.error || "Falha ao carregar datas");
        if (cancelled) return;
        setAvailableDates(new Set((json.dates || []).filter(Boolean)));
      } catch (e) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function run() {
      if (!selectedExameId || !selectedDate) {
        setSlots([]);
        return;
      }
      setLoadingDispon(true);
      try {
        const res = await fetch(
          `/api/agendamentos/disponibilidade?exameId=${encodeURIComponent(String(selectedExameId))}&date=${encodeURIComponent(
            selectedDate,
          )}`,
          { cache: "no-store" },
        );
  ...
  const calendar = useMemo(() => {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (dayIndex < 1) {
        ...
        cells.push({
          iso: isoFor(prev.getFullYear(), prev.getMonth() + 1, dd),
          label: String(dd),
          muted: true,
        });
        ...
        cells.push({
          iso: isoFor(next.getFullYear(), next.getMonth() + 1, dd),
          label: String(dd),
          muted: true,
        });
      } else {
        cells.push({
          iso: isoFor(year, month + 1, dayIndex),
  ...
  async function confirmAgendamento() {
    ...
    try {
      ...
      setTab("Agendada");
      // força refresh
      const listRes = await fetch(`/api/agendamentos?cpf=${encodeURIComponent(cpf)}&status=Agendada&tipo=exame`, {
        cache: "no-store",
      });
      const listJson = (await listRes.json()) as { data?: AgendamentoRow[] };
      setAgendamentos(listJson.data || []);
      ...
      <div className={styles.content} aria-busy={loading || loadingAg || saving}>
        {step !== "lista" ? (
          <div className={styles.headerRow}>
            ...
            <div className={styles.title14}>Nova Consulta</div>
        ...
        {cancelAgOpen && cancelAgId ? (
          <div className={styles.sheetOverlay} role="dialog" aria-modal="true" aria-label="Confirmar cancelamento">
            <div className={styles.sheet}>
              <div className={styles.sheetTitle}>Cancelar exame agendado?</div>
              <div style={{ height: 8 }} />
              <div className={styles.muted}>Você tem certeza que deseja cancelar este exame?</div>
              <div style={{ height: 16 }} />
          ...
          <>
            ...
            <div className={`${styles.card} ${styles.cardPad24Y}`}>
              <div className={`${styles.cardInner24} ${styles.inputBlock}`}>
                <div className={styles.inputLabel}>Tipo de Exame</div>
                <div className={styles.selectLike} aria-label="Selecionar exame">
                  <select
                    className="ds-control ds-md"
                    value={selectedExameId ? String(selectedExameId) : ""}
                    onChange={(e) => {
                      const nextId = Number(e.target.value);
                      const next = exames.find((x) => x.id === nextId) || null;

                      setSelectedExameId(next?.id ?? null);
                      setSelectedEspecialidade(next?.nome ?? "");

                      // reset do restante do fluxo ao trocar a especialidade
                      setSelectedDate("");
                      setSelectedHorario("");
                      setSelectedLocalId(null);
                      setSelectedLocal("");
                    }}
                    disabled={catalogoLoading || !exames.length}
                  >
                    <option value="" disabled>
                      {catalogoLoading ? "Carregando..." : "Selecione o exame"}
                    </option>
                    {exames.map((e) => (
                ...
                <button
                  ...
                  type="button"
                  disabled={!selectedEspecialidade || !selectedExameId}
                  onClick={() => {
                    // garante que o usuário caia em uma data válida
                    const first = Array.from(availableDates.values()).sort()[0] || "";
          ...
          <>
            <div className={`${styles.card} ${styles.cardPad24Y}`}>
              <div className={styles.cardInner24}>
                ...
                <div className={styles.value12}>{selectedEspecialidade || "-"}</div>
              </div>
            </div>
            ...
            <div className={`${styles.card} ${styles.cardPad16}`}>
              ...
              <div className={styles.daysGrid} role="grid" aria-label="Calendário">
                {calendar.cells.map((c) => {
                  ...
                  const cls = [
                    styles.day,
                    c.muted ? styles.dayMuted : "",
                    isAvail ? styles.dayAvailable : "",
        ...
        >
          <div className={styles.sheet} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={`${styles.card} ${styles.cardPad24Y}`}>
              ...
              <div className={styles.fieldRow}>
                <div className={styles.label10}>Especialidade:</div>
                <div className={styles.fieldValue}>{selectedEspecialidade || "-"}</div>
              </div>
              <div className={styles.fieldRow}>
                <div className={styles.label10}>Data da Consulta:</div>
