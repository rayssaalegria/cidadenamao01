...
export async function GET(req: Request) {
  ...
  async function list(select: string) {
    for (const c of candidates) {
      const { ok, status, json, text } = await supabaseRestJson<ReceitaRow[]>(
        `/rest/v1/receitas?select=${encodeURIComponent(select)}&cpf=eq.${encodeURIComponent(c)}&order=created_at.desc`,
        { method: "GET" }
      );
      if (!ok) return { ok, status, json, text };
      if (Array.isArray(json) && json.length) return { ok, status, json, text };
    }
    return { ok: true, status: 200, json: [] as ReceitaRow[], text: "" };
  }
  ...
  if (!res.ok) {
    const msg = String(res.text || "Erro ao carregar receitas");
  ...
  return NextResponse.json({ data: (res.json || []) as ReceitaRow[] }, { status: 200 });
}
...
export async function POST(req: Request) {
  ...
  let r = await supabaseRestJson<{ id: number; cpf: unknown }[]>(
    `/rest/v1/receitas?select=id,cpf`,
    {
      method: "POST",
  ...
  if (!r.ok) return NextResponse.json({ error: r.text || "Erro ao criar receita" }, { status: 500 });
